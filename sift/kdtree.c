...
*/
struct kd_node* kdtree_build( struct feature* features, int n )//建立kd tree
{
					...
					struct feature*** nbrs, int max_nn_chks )
{
	...
	_nbrs = calloc( k, sizeof( struct feature* ) );// k number of neighbors to find
	min_pq = minpq_init();
	...
	while( min_pq->n > 0  &&  t < max_nn_chks )
	{
		...
		for( i = 0; i < expl->n; i++ )
		{
			...
			//按从小到大塞到neighbor数组里，到时候取前k个就是 KNN 咯~ n 每次加1或0，表示目前已有的元素个数
...
static struct kd_node* kd_node_init( struct feature* features, int n )
{
	...
	kd_node = malloc( sizeof( struct kd_node ) );//分配一块大小为kd_node的内存区域，是kd_node指针指向这块区域
	memset( kd_node, 0, sizeof( struct kd_node ) );
	kd_node->ki = -1;
	kd_node->features = features;//初始化此节点的特征
	kd_node->n = n;//特征的数目
...
static void expand_kd_node_subtree( struct kd_node* kd_node )
{
	...
	assign_part_key( kd_node );//找到用来划分的描述子索引及其相应的值，依据是var最大的一个最容易划分
	partition_features( kd_node );//以其值和索引进行划分

	if( kd_node->kd_left )//递归划分
...
static void assign_part_key( struct kd_node* kd_node )
{
	...
	for( i = 0; i < n; i++ )
		tmp[i] = features[i].descr[ki];//double=double
...
static void partition_features( struct kd_node* kd_node )
{
	...
	for( i = 0; i < n; i++ )
		...
		{
			tmp = features[++j];//把小于kv的features放到features[++j]中，即features[0,1,2,3,4,p]
			features[j] = features[i];
			features[i] = tmp;
			if( features[j].descr[ki] == kv )
				p = j;
		}
	tmp = features[p];//处理这样的情况 比如kv=4 数据为1 5 2 4 3 2 6 7，这样下来j=4，p=3；1 2 2 4 3 5 6 7，features[p]=4,features[j]=3,所以调换顺序
										...
										struct min_pq* min_pq )
{
	...
	while( expl  &&  ! expl->leaf )
	{
		...
		if( minpq_insert( min_pq, unexpl, ABS( kv - feat->descr[ki] ) ) )//用kv-feat->descr[ki]作为优先级
		{
								  ...
								  int n, int k )
{
	...
	else//n>=k 比如k=2,n=3,则恢复n-1的feature_data数据
	{
		nbrs[n-1]->feature_data = ndata->old_data;
		free( ndata );
	}
	i = n-2;//上面处理了n-1,下面程序依次处理n-2 n-3
	while( i >= 0 )
	{
