...
static IplImage* create_init_img( IplImage* img, int img_dbl, double sigma )
{
	...
	{
		//assume that the original image has a blur of at least 0.5,and that therefore
		//the doubled image has sig=1
...
{
	IplImage*** gauss_pyr;
	double* sig = calloc( intvls + 3, sizeof(double));//会将所分配的内存空间中的每一位都初始化为零
	...
	*/
	...
	for( i = 1; i < intvls + 3; i++ )
	{
		...
		//直接做滤波的话计算花费大，这么做可以在前一级的基础上滤波。滤波的核函数小，速度快
		sig[i] = sqrt( sig_total * sig_total - sig_prev * sig_prev );
	}

	for( o = 0; o < octvs; o++ )
		for( i = 0; i < intvls + 3; i++ )//3+3
		{
			if( o == 0  &&  i == 0 )
				gauss_pyr[o][i] = cvCloneImage(base);

			/* base of new octvave is halved image from end of previous octave */
			else if( i == 0 )//每组金字塔的第一幅图像
				gauss_pyr[o][i] = downsample( gauss_pyr[o-1][intvls] );//上一层的中间图像2sign，第intvls幅，k=pow（2,1/intvls）刚好为二倍下采样高斯模糊的系数，直接下采样省去高斯模糊
...
{
	CvSeq* features;
	double prelim_contr_thr = 0.5 * contr_thr / intvls;//0.04*0.5/3
	struct feature* feat;
	...
	for( o = 0; o < octvs; o++ )//
		for( i = 1; i <= intvls; i++ )//1-3
			//SIFT_IMG_BORDER width of border in which to ignore keypoints
										...
										int r, int c, int intvls, double contr_thr )
{
	...
	if( ABS( contr ) < contr_thr / intvls )//0.04/3
		return NULL;

	feat = new_feature();
	ddata = feat_detection_data( feat );//#define feat_detection_data(f) ( (struct detection_data*)(f->feature_data) )
...
/*
Calculates interpolated pixel contrast.  Based on Eqn. page11 in Lowe's paper.
	...
	for( i = -rad; i <= rad; i++ )
		for( j = -rad; j <= rad; j++ )
			if( calc_grad_mag_ori( img, r + i, c + j, &mag, &ori ) )
			{
				...
				bin = cvRound( n * ( ori + CV_PI ) / PI2 );//+CV_PI将-pi~pi转换到0-2*pi
				bin = ( bin < n )? bin : 0;
				hist[bin] += w * mag;
			}
								   ...
								   double mag_thr, struct feature* feat )
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		if( hist[i] > hist[l]  &&  hist[i] > hist[r]  &&  hist[i] >= mag_thr )
		{
			...
			new_feat->ori = ( ( PI2 * bin ) / n ) - CV_PI;//之前加过一个CV_PI，还原真实值
...
*/
static void compute_descriptors( CvSeq* features, IplImage*** gauss_pyr, int d, int n)//d宽度，本程序中为4；n为bin的个数，本程序中为8
{
	struct feature* feat;
	struct detection_data* ddata;
	double*** hist;
							 ...
							 double scl, int d, int n )
{
	...
	hist = calloc( d, sizeof( double** ) );//计算4*4个hist，每个hist有8个方向，所以共有128维向量
	for( i = 0; i < d; i++ )
	{
	...
	//printf("hist_width= %f",hist_width);//6.*
	//参考博文http://blog.csdn.net/xiaowei_cqu/article/details/8113565
	...
	for( i = -radius; i <= radius; i++ )
		for( j = -radius; j <= radius; j++ )
		{
			...
			*/
			/*以新坐标表示旧坐标 除以小窗口的宽度的原因是归一化当前描述子,即以
			描述子为坐标原点，左右的范围各位-d/2 d/2,格子和格子之前插值按照中心点来算*/
