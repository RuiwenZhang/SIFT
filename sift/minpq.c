...
{
	struct min_pq* min_pqs;

	//min_pqs =static_cast<min_pq *>(malloc( sizeof( struct min_pq ) ));//c编译器中不能用c++的语法
	min_pqs =malloc( sizeof( struct min_pq ) );
	min_pqs->pq_array = calloc( MINPQ_INIT_NALLOCD, sizeof( struct pq_node ) );
	min_pqs->nallocd = MINPQ_INIT_NALLOCD;
	min_pqs->n = 0;
